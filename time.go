...
// TimeEncodingLen is the length of byte slice time representation
// returned by EncodeTime.
const TimeEncodingLen = 9

// EncodeTime returns slice of bytes that represent
// provided time in UTC. Slice length is always 9, where
// first 2 bytes represent the year and the rest 7 bytes
// nanoseconds since the beginning the year.
...
// MarshalTime puts bytes representation if provided time
...
// DecodeTime converts slice of bytes as described in encodeTime
// to time.Time.
